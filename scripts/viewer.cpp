...
void Callback(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
    ...
    faceDetector.detectMultiScale((cv_bridge::toCvShare(msg, "bgr8")->image) , faces, 1.1, 4, CASCADE_SCALE_IMAGE, Size(30, 30));

    for (int i = 0; i < faces.size(); i++){ //for locating the face
         Mat faceROI = cv_bridge::toCvShare(msg, "bgr8")->image(faces[i]);//Storing face in the matrix//
         ...
         rectangle(cv_bridge::toCvShare(msg, "bgr8")->image , Point(x, y), Point(w, h), Scalar(255, 0, 255), 2, 8, 0);//Drawing a rectangle using around the faces//
      }

    imshow("stream", cv_bridge::toCvShare(msg, "bgr8")->image);  //cv_bridge::toCvShare(msg, "bgr8")->image);
